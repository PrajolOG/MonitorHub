...
// import com.MonitorHub.model.User; // For potential use

import java.sql.*;
import java.math.BigDecimal;
import java.time.LocalDateTime;
import java.time.ZoneId; // Import ZoneId for conversion
...
public class OrderDAO {
    ...
    // Method to save an order (can be part of a transaction)
    public long saveOrder(Order order, Connection conn) throws SQLException {
        ...
        // The PreparedStatement will be closed by the try-with-resources block in the calling method (e.g., CheckoutServlet)
        // if conn is passed from outside and managed there.
        // If this method were to get its own connection, it should close it.
        ...
        try {
            ...
            try (ResultSet generatedKeys = pstmt.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    ...
                    order.setOrderId((int) generatedSaleId); // Assuming orderId in Order model is int
                } else {
            ...
            // Connection 'conn' is managed by the caller, so it's not closed here
        }
    ...
    public BigDecimal getTotalSalesValue() throws SQLException {
        ...
        try {
            ...
            DatabaseConfig.closeResources(conn, stmt, rs); // Use your helper
        }
        return totalRevenue;
    }
    ...
    public List<Map<String, Object>> getRecentSalesForDashboard(int limit) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Convert Timestamp to LocalDateTime for consistency if other parts of your app use LocalDateTime
                Timestamp saleTimestamp = rs.getTimestamp("sale_date");
                if (saleTimestamp != null) {
                    item.put("orderDate", saleTimestamp.toLocalDateTime());
                } else {
            ...
            DatabaseConfig.closeResources(conn, pstmt, rs); // Use your helper
        }
    ...
    public List<Order> getAllOrders() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                order.setOrderId((int) rs.getLong("saleid")); // Assuming orderId is int in model
                ...
                if (saleTimestamp != null) {
                    ...
                    order.setOrderDate(saleDateTime); // Set original LocalDateTime

                    // **NEW: Convert and set the java.util.Date for JSP**
                    order.setOrderDateForJsp(
                        java.util.Date.from(saleDateTime.atZone(ZoneId.systemDefault()).toInstant())
                    );
                }
                // For simplicity, assuming all fetched orders are "Completed"
                // You might fetch status from DB if it exists
